...
extern "C" RF24* CreateRadio(uint8_t _cepin, uint8_t _cspin, uint32_t spispeed)
{
    return new RF24(_cepin, _cspin, spispeed);
}
...
extern "C" void read(RF24* radio, void* buf, uint8_t len)
{
    if(radio != NULL)
    {
        radio->read(buf, len);
...
extern "C" bool writeFast(RF24* radio, const void* buf, uint8_t len)
{
    if(radio != NULL)
    {
